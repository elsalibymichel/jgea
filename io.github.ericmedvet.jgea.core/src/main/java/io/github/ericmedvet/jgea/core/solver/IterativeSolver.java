...
public interface IterativeSolver<T extends State<P, S>, P extends Problem<S>, S> extends Solver<P, S> {
  ...
  Collection<S> extractSolutions(P problem, RandomGenerator random, Executor executor, T state) throws SolverException;

  T init(P problem, RandomGenerator random, Executor executor) throws SolverException;

  boolean terminate(RandomGenerator random, Executor executor, T state) throws SolverException;

  T update(RandomGenerator random, Executor executor, T state) throws SolverException;

  @Override
  default Collection<S> solve(P problem, RandomGenerator random, Executor executor) throws SolverException {
